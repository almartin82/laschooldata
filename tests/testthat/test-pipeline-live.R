# ==============================================================================
# LIVE Pipeline Tests for laschooldata
# ==============================================================================
#
...
test_that("Louisiana DOE website is accessible", {

  skip_if_offline()
  skip("TODO: Add Louisiana DOE URL and verify HTTP 200")
})
...
test_that("Can download Louisiana enrollment data file", {
  skip_if_offline()
...
test_that("Can parse Louisiana enrollment file", {
  skip_if_offline()
...
test_that("laschooldata data file has expected columns", {
...
test_that("get_raw_enr returns data for valid year", {
  ...
  # Get available years
  years_info <- laschooldata::get_available_years()
  ...
  tryCatch({
    raw <- laschooldata:::get_raw_enr(test_year)
...
test_that("get_available_years returns valid year range", {
  result <- laschooldata::get_available_years()
...
test_that("fetch_enr returns data with no Inf or NaN", {
  ...
  tryCatch({
    ...
    data <- laschooldata::fetch_enr(test_year, tidy = TRUE)
...
test_that("Enrollment counts are non-negative", {
  ...
  tryCatch({
    ...
    data <- laschooldata::fetch_enr(test_year, tidy = FALSE)
...
test_that("tidy=TRUE and tidy=FALSE return consistent totals", {
  ...
  tryCatch({
    ...
    wide <- laschooldata::fetch_enr(test_year, tidy = FALSE)
    tidy <- laschooldata::fetch_enr(test_year, tidy = TRUE)
...
test_that("Cache functions exist and work", {
  ...
  tryCatch({
    path <- laschooldata:::get_cache_path(2024, "enrollment")
