...
def get_test_years():
    ...
    if _available_years is None:
        ...
        _available_years = la.get_available_years()
...
class TestImport:
    ...
    def test_import_functions(self):
        ...
        import pylaschooldata as la
        assert hasattr(la, 'fetch_enr')
        assert hasattr(la, 'fetch_enr_multi')
        assert hasattr(la, 'tidy_enr')
        assert hasattr(la, 'get_available_years')
...
class TestFetchEnr:
    ...
    def test_returns_dataframe(self):
        ...
        df = la.fetch_enr(years['max_year'])
    ...
    def test_total_enrollment_reasonable(self):
        ...
        years = get_test_years()
        df = la.fetch_enr(years['max_year'])
        # Total enrollment in Louisiana varies by grade level aggregation
        # Just check that total is positive and reasonable
        total = df['n_students'].sum()
        # Louisiana should have substantial enrollment
...
class TestFetchEnrMulti:
    ...
    def test_returns_dataframe(self):
        ...
        df = la.fetch_enr_multi([years['max_year']])
        assert isinstance(df, pd.DataFrame)
    ...
    def test_contains_requested_year(self):
        ...
        df = la.fetch_enr_multi([test_year])
    ...
    def test_multi_matches_single(self):
        ...
        import pylaschooldata as la
        years = get_test_years()
        df_single = la.fetch_enr(years['max_year'])
        df_multi = la.fetch_enr_multi([years['max_year']])
